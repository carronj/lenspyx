...
def _get_mf_response_old(qe_key:str, nlev_t:float, beam:float, lmax_ivf:int, lmax_sky:int, cls_unl:dict,
    ...
    if inoise_cls is None:
        inoise_cls = dict()
    if 'tt' not in inoise_cls and qe_key in ['ptt', 'p']:
    ...
    for spec in ['ee', 'bb']:
        if spec not in inoise_cls and qe_key in ['p_p', 'p']:
