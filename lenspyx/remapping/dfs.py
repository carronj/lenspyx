...
def gclm2dfs(gclm, mmax, spin, ringw=None, ntheta=None, numthreads=0, verbose=True, scale=1):
    ...
    mmax_eff = int(np.ceil(st2mmax(spin, thtmax, ntheta)))
    ...
    print("Setting up DFS grid Nt Np %s %s (effective mmax %s %s)"%(ntheta_dfs, nphi, mmax_eff, int( (ntheta * np.sin(thtmax)))))


    # relevant map values, that we will spread onto the DFS map:
    tim.start('experimental.synthesis')
    map = cc_geom.synthesis(gclm, spin, lmax_unl, mmax, numthreads,  mode=ducc_sht_mode(gclm, spin))
    tim.close('experimental.synthesis')

    # We patch them onto the doubled fourier sphere
    tim.start('dfs map build')
    tim.start('--> zeroes')
    ...
    tim.start('--> filling north')
    # FIXME: this assumes contiguous values in nzrorings
    rmin, rmax = nzro_rings[0], nzro_rings[-1]
    if spin == 0:
        map_dfs[rmin:rmax + 1, :] = map[0].reshape(cc_geom.theta.size, nphi)
    else:
        # FIXME: this assumes contiguous values in nzrorings
        map_dfs[rmin:rmax + 1, :] = (map[0] + 1j * map[1]).reshape(cc_geom.theta.size, nphi)
    tim.close('--> filling north')

    del map
    assert ringw.size == ntheta
    tim.start('--> map weighting')
    for ir, w in zip(nzro_rings, ringw[nzro_rings]): # Applies (eg apodization) weights to the DFS map
        map_dfs[ir, :] *= w
    tim.close('--> map weighting')
    tim.start('--> filling south')
    # TODO: could fill only non-zero rings
    map_dfs[ntheta_dfs:, :nphihalf] = map_dfs[ntheta_dfs - 2:0:-1, nphihalf:]
    map_dfs[ntheta_dfs:, nphihalf:] = map_dfs[ntheta_dfs - 2:0:-1, :nphihalf]
    tim.close('--> filling south')

    if (spin % 2) != 0:
        map_dfs[ntheta_dfs:, :] *= -1
    tim.close('dfs map build')

    # go to Fourier space
    tim.start('map_dfs 2DFFT')
    if spin == 0:
...
def gclm2dfs_v2(gclm, mmax, spin, ringw=None, ntheta=None, numthreads=0, verbose=0, scale=1):
    """Send the input gclm array to a DFS map
    ...
    #FIXME: this does not work, but is there mostly to test the timings
    """
    tim = timer(True, prefix='gclm2dfs')
    gclm = np.atleast_2d(gclm)
    lmax_unl = Alm.getlmax(gclm[0].size, mmax)
    ...
    # relevant map values, that we will spread onto the DFS map:
    #map = cc_geom.synthesis(gclm, spin, lmax_unl, mmax, numthreads,  mode=ducc_sht_mode(gclm, spin))
    tht = np.linspace(0, np.pi, ntheta)[np.where(ringw > 0)]
    # FIXME: can reduce m ?
    print(ducc_sht_mode(gclm, spin))
    tim.start('experimental.alm2leg')
    legs = ducc0.sht.alm2leg(alm=gclm, lmax=lmax_unl, theta=tht, spin=spin,
                             nthreads=numthreads, mode= ducc_sht_mode(gclm, spin), theta_interpol=False)
    tim.close('experimental.alm2leg')

    #leg = alm2leg(alm=gclm, spin=abs(s2), lmax=lmax, mval=np.array([abs(s1)], dtype=int),
    #              mstart=np.array([0]), theta=theta, mode=mode).squeeze()
    # We patch them onto the doubled fourier sphere
    tim.start('map_dfs build')
    map_dfs = np.zeros((2 * ntheta_dfs - 2, nphi), dtype=complex)
    # :Use 1d exluding rings for non trivial weights
    #map = ducc0.sht.experimental.synthesis_2d(alm=gclm, ntheta=ntheta, nphi=nphi,
    #                                          spin=spin, lmax=lmax_unl, mmax=mmax, geometry="CC", nthreads=numthreads,
    #                                          mode=mode)
    ...
    if spin == 0:
        # FIXME: this assumes contiguous values in nzrorings
        # FIXME: needs to make m match, reality condition ?
        map_dfs[nzro_rings, :mmax_eff+1] = legs[0, :, :mmax_eff+1]
    else:
        # FIXME: this assumes contiguous values in nzrorings
        map_dfs[nzro_rings, :mmax_eff+1] = (legs[0, :, :mmax_eff+1] + 1j * legs[1, :, :mmax_eff+1])
    del legs
    assert ringw.size == ntheta
    for ir, w in zip(nzro_rings, ringw[nzro_rings]): # Applies (eg apodization) weights to the DFS map
        map_dfs[ir, :] *= w
    map_dfs[ntheta_dfs:, :nphihalf] = map_dfs[ntheta_dfs - 2:0:-1, nphihalf:]
    map_dfs[ntheta_dfs:, nphihalf:] = map_dfs[ntheta_dfs - 2:0:-1, :nphihalf]
    #map_dfs[ntheta_dfs:, :] = map_dfs[ntheta_dfs - 2:0:-1, :] # is this right ?
    if (spin % 2) != 0:
        map_dfs[ntheta_dfs:, :] *= -1
    tim.close('map_dfs build')

    # go to Fourier space
    tim.start('map_dfs 1DFFT')
    if spin == 0:
        tmp = np.empty(map_dfs.shape, dtype=ctype[map_dfs.dtype])
        map_dfs = ducc0.fft.c2c(map_dfs, axes=(0,), inorm=2, nthreads=numthreads, out=tmp)
        del tmp
    else:
        map_dfs = ducc0.fft.c2c(map_dfs, axes=(0,), inorm=2, nthreads=numthreads, out=map_dfs)
    tim.close('map_dfs 1DFFT')
    if verbose:
        print(tim)
    # For convenience, tht coordinates of DFS points, and rescaling factor to convert tht of spherical map to tht of dfs
    tht_dfs = np.linspace(0, 2 * np.pi, ntheta_dfs * 2 - 2, endpoint=False)
