...
if __name__ == '__main__':
    ...
    gls = [utils_geom.Geom.get_thingauss_geometry(lmax_sky + dlmax_gl, args.spin),
           utils_geom.Geom.get_gl_geometry(lmax_sky + dlmax_gl, True),
    ...
    gls_lab = ['thGL', 'GL no thin', 'F1 lmax + 2', 'F1 wMRtrick','F1 lmax + 2 (no thinning)', 'HP same #pix as GL'] # 'CC lmax + 2', 'CC wMRtrick' # Cant uses on the poles for now
    ...
    for igeom, (gl, gl_lab) in enumerate(zip(gls, gls_lab)):
        ...
        for i, lab in zip(range(ncomp), [gl_lab + ' G (%.2f Mpix)' % (gl.npix() / 1e6), gl_lab + 'C']):
            diff = alm_out[i] - alm_ref[i]
            if igeom == 0:
                clres_ref = alm2cl(diff, diff, lmax_wted, mmax_wted, lmax_wted)
            else:
                this_ratio = np.sqrt(alm2cl(diff, diff, lmax_wted, mmax_wted, lmax_wted)[ls] / clres_ref[ls])
